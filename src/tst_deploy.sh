...
GENERATED_DIR=$repo_path/doc
NEW_GENERATED_DIR=$repo_path/autogen-docs
...
# Helper
update_subtree () {
  if [ x"$gh_pages" = x"$(git -C $repo_path rev-parse --symbolic-full-name --abbrev-ref HEAD)" ]; then
    if [ -d "$1" ]; then
      git -C $repo_path merge -Xsubtree="$1" -Xtheirs "$2" -m "$3"
      echo "merged $2 in $1"
    else
      git -C $repo_path subtree add --prefix="$1" "$2" -m "$3"
      echo "added $2 in $1"
    fi
  fi
}
...
handle_nojekyll_file() {
  if [ -f .nojekyll ]; then
     ...
     rm .nojekyll
     git -C $repo_path commit .nojekyll -m "Removed .nojekyll file"
  fi
}
...
handle_old_autogenerated_docs() {
  if [ -d "$GENERATED_DIR" ]; then
    ...
    git -C $repo_path add $GENERATED_DIR $NEW_GENERATED_DIR
    git -C $repo_path commit -m "Moved old docs to $NEW_GENERATED_DIR"
  fi
}

get_subtree () {
  git -C $repo_path add -f $1 >> /dev/null
  git -C $repo_path commit -m "Added $1 version $2" >> /dev/null
  hash=$(git -C $repo_path subtree split --prefix=$1)
  echo $hash
}
  ...
  for version in $versions; do
    if [ x"" != x"$(echo "$version" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+$")" ]; then
      ...
      if [ -d doc ]; then
        ...
        git -C $repo_path add -f doc/ ; git -C $repo_path commit --allow-empty -m "Clean build artifacts for version $version"
        git -C $repo_path checkout $gh_pages --force
        ...
        git -C $repo_path commit -m "Updated docs for $version"
      fi
    fi
  done
  git -C $repo_path checkout $gh_pages --force
...
commit_jekyll_data() {
  git -C $repo_path add _data/*
  git -C $repo_path commit -m "Updated global toc and version"
  git -C $repo_path add --all $DOCS_ROOT/latest
  git -C $repo_path commit --allow-empty -m "Updated latest"
}

# Merges files from $1 (should be latest version tag) into gh-pages branch
update_jekyll_structure() {
  git -C $repo_path checkout "$1"
  ...
  branch_available=$(git -C $repo_path branch --list $gh_pages)
  GH_PAGES_STUB=$(git -C $repo_path subtree split --prefix=gh-pages-stub/)
  if [ x"" =  x"$branch_available" ]; then
    git -C $repo_path checkout "${SERVER_NAME}/$gh_pages" -b "$gh_pages"
  else
    git -C $repo_path checkout "$gh_pages"
  fi
  echo_green "Merging gh-pages-stub from $1"
  git -C $repo_path merge -Xtheirs "$GH_PAGES_STUB" -m "Merged jekyll files"
}
...
deploy_docs () {
  ...
  latest=$(git -C $repo_path tag | sort -V -r | head -1)
...
# Test docs
test_docs() {
  curr_tag=$(git -C $repo_path tag --contains)
  ...
  if [ ! -d $jekyll_root ]; then
    ...
    echo "$repo_path/doc"
    echo "$jekyll_root/$DOCS_ROOT/$curr_tag"
    ln -s "$repo_path/doc" "$jekyll_root/$DOCS_ROOT/$curr_tag"
  fi
  cd $jekyll_root
  generate_jekyll_data "$latest"
